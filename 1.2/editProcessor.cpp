...
//    openFileAction = new QAction(QIcon(":/new/prefix1/img/open.png"),tr("打开"),this);//在创建"打开文件"动作的同时,指定了此动作使用的图标\名称及父窗口
    ...
    task_1_Btn = new QToolButton;
    task_1_Btn->setIcon(QIcon(":/new/prefix1/img/task.png"));
    task_1_Btn->setToolTip(tr("作业功能"));
    task_1_Btn->setCheckable(true);
...
//    connect(listComboBox,SIGNAL(activated(int)),this,SLOT(ShowList(int)));
    ...
    connect(task_1_Btn,SIGNAL(clicked(bool)),this,SLOT(ShowTask()));
}
...
void EditProcessor::createActions()
{
    ...
    // "退出" 动作
    exitAction = new QAction(QIcon(":/new/prefix1/img/exit..png"),tr("退出"),this);
...
//    connect(aboutAction,SIGNAL(triggered(bool)),this,SLOT(QApplication::aboutQt()));
    ...
    coffeeAction = new QAction(tr("关于作者"),this);

    frame = new QFrame;
    frame->resize(922,600);
    QPixmap pixmap(":/new/prefix1/img/pay.jpg");//设定图片
    pixmap = pixmap.scaled(QSize(922,600),Qt::KeepAspectRatio);
    palette=new QPalette;//创建一个调色板对象
    ...
    connect(coffeeAction,SIGNAL(triggered(bool)),this,SLOT(showCoffee()));
...
//    zoomMenu->addAction(aboutAction);
    ...
    aboutMenu->addAction(coffeeAction);
}
...
//    listToolBar->addWidget(leftAction);
//    listToolBar->addWidget(rightAction);
//    listToolBar->addWidget(centerAction);
//    listToolBar->addWidget(justifyAction);
    listToolBar = addToolBar("List");
    listToolBar->addAction(leftAction);
    listToolBar->addAction(rightAction);
    listToolBar->addAction(centerAction);
    listToolBar->addAction(justifyAction);
    /**************************************/
    taskToolBar = addToolBar("Task");
    taskToolBar->addWidget(task_1_Btn);
...
//                  textStream.setCodec(QTextCodec::codecForName("UTF-8"));
                  ...
                  showWidget->text->setPlainText(ReadyText);
...
//                  }
                  ...
                  setWindowTitle(("My Word                                     "+fileName));
...
void EditProcessor::ShowTask()
{

    task->setWindowFlags(Qt::FramelessWindowHint);
    task->printNums();
}
